...
export default async function BrandDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <AppShell user={user}>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6"><Metric label="Pages" value={pages.length} icon={FileTextIcon} accent="text-violet-500" /><Metric label="Generated" value={(counts.generated ?? 0) + (counts.approved ?? 0) + (counts.scheduled ?? 0) + (counts.published ?? 0)} icon={SparklesIcon} accent="text-blue-500" /><Metric label="Approved" value={(counts.approved ?? 0) + (counts.scheduled ?? 0) + (counts.published ?? 0)} icon={CheckCircleIcon} accent="text-ok" /><Metric label="Published" value={counts.published ?? 0} icon={GlobeIcon} accent="text-orange-500" /><Metric label="QA pass" value={`${qaPass}/${qaTotal || 0}`} icon={ShieldCheckIcon} accent="text-teal-500" /><Metric label="Gen cost" value={`$${Number(cost).toFixed(2)}`} icon={DollarIcon} accent="text-amber-500" /></div>
      <SiteConsoleTabs>
...
function AuditLog({ events }: { events: any[] }) {
  return <div><h2 className="mb-2 font-semibold">Audit log</h2><div className="card"><div className="grid grid-cols-1 gap-y-5 sm:grid-cols-2 lg:grid-cols-4 lg:gap-x-8">{events.length === 0 && <p className="text-sm text-dim sm:col-span-2 lg:col-span-4">No events yet.</p>}{events.map((e: any, i: number) => <div key={i} className="relative flex min-w-0 items-start gap-3 lg:after:absolute lg:after:right-[-21px] lg:after:top-1/2 lg:after:text-lg lg:after:text-faint lg:after:content-['→']"><span className="mt-0.5 rounded-lg bg-primary/10 p-2 text-primary"><ActivityIcon size={17} /></span><div className="min-w-0"><div className="text-sm font-semibold text-ink">{e.event_type}</div><div className="mt-1 text-xs text-faint">{new Date(e.created_at).toLocaleTimeString()}</div>{e.detail?.path && <div className="mt-1 truncate font-mono text-[10px] text-faint">{e.detail.path}</div>}</div></div>)}</div></div></div>;
}

function Metric({ label, value, icon: Icon, accent }: { label: string; value: number | string; icon: (props: { size?: number; className?: string }) => React.JSX.Element; accent: string }) {
  return <div className="card relative py-3"><span className={`absolute right-3.5 top-3.5 ${accent}`}><Icon size={19} /></span><div className="stat-num">{value}</div><div className="stat-label">{label}</div></div>;
}
